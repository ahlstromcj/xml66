...
CYGWIN=binmode
export CYGWIN
export XML66_SCRIPT_EDIT_DATE="2026-02-20"
export XML66_LIBRARY_API_VERSION="0.1"
export XML66_LIBRARY_VERSION="$XML66_LIBRARY_API_VERSION.0"
export XML66="xml66"
export XML66_LIBRARY="$XML66-$XML66_LIBRARY_API_VERSION"
...
if test "$DOVERSION" = "yes" ; then
   echo "Cfg66 version $XML66_LIBRARY_VERSION $XML66_SCRIPT_EDIT_DATE"
   exit 0
fi
...
if test "$DOHELP" = "yes" ; then
   ...
   cat << E_O_F
Usage: ./work [options]    ($XML66_LIBRARY_VERSION-$XML66_SCRIPT_EDIT_DATE)
...
be more to come. Some options might not work on Windows.
 ...
 --clean             Delete the usual derived files from the project. Also
                     do "git checkout doc/xml66-library-guide.pdf"
...
if test $DOCLEAN = "yes" ; then
   ...
   rm -f doc/latex/*.log
   rm -rf build/subprojects/
   echo "Build products removed from the xml66/build directory."
...
#  rm -f doc/latex/*.log
#  echo "Build products removed from the xml66/build directory."
#  git checkout doc/xml66-library-guide.pdf tests/data/fooout.rc
#  echo "Previous version of developer guide restored."
fi
...
# https://mesonbuild.com/Running-Meson.html
   ...
   echo "Making all xml66 libraries..."
...
if test "$DOINSTALL" = "yes" ; then
   ...
   if test "$USERID" = 0 ; then
      cd build
      echo "Installing the xml66 library..."
      meson install
      cd ..
   else
      echo "UID $USERID. We want you as root to install the xml66 library..."
   fi
...
if test "$DOUNINSTALL" = "yes" ; then
   ...
   USERID=$(id -u)
   if test "$USERID" = 0 ; then
      echo "Uninstalling the xml66 library..."
      ninja -C build uninstall
      if test "$PLATFORM" = "UNIX" ; then
         rm -rf "$INSTALL_PREFIX/include/$XML66_LIBRARY"
         rm -rf "$INSTALL_PREFIX/$INSTALL_LIBDIR/$XML66_LIBRARY"
         rm -rf "$INSTALL_PREFIX/share/doc/$XML66"
#        rm -rf "$INSTALL_PREFIX/man/man1/$XML66.1"
      fi
   else
      echo "UID $USERID. We want you as root to uninstall the xml66 library..."
   fi
