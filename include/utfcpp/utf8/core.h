#if ! defined UTFCPP_UTF8_CORE_H
#define UTFCPP_UTF8_CORE_H
 ...
 *
 * Chris Ahlstrom 2026. Some application of later C++ standards.
 */
...
#define UTF_CPP_STATIC_ASSERT(condition) static_assert \
   (condition, "UTFCPP static assert");
#else                                   // C++ 98/03
...
/**
 * Helper code - not intended to be directly called by the library users. May
...
inline bool
is_lead_surrogate (utfchar32_t cp)
{
    return
    (
        cp >= static_cast<utfchar32_t>(LEAD_SURROGATE_MIN) &&
        cp <= static_cast<utfchar32_t>(LEAD_SURROGATE_MAX)
    );
}

inline bool
is_trail_surrogate (utfchar32_t cp)
{
    return
    (
        cp >= static_cast<utfchar32_t>(TRAIL_SURROGATE_MIN) &&
        cp <= static_cast<utfchar32_t>(TRAIL_SURROGATE_MAX)
    );
}

inline bool
is_surrogate (utfchar32_t cp)
{
    return
    (
        cp >= static_cast<utfchar32_t>(LEAD_SURROGATE_MIN) &&
...
inline bool
is_code_point_valid (utfchar32_t cp)
{
    return (cp <= CODE_POINT_MAX && !utf8::internal::is_surrogate(cp));
}

inline bool
is_in_bmp (utfchar32_t cp)
{
...
inline bool
is_overlong_sequence (utfchar32_t cp, int length)
{
    if (cp < 0x80)
    {
        if (length != 1)
...
template <typename octet_iterator>
utf_error
increase_safely (octet_iterator & it, const octet_iterator ender)
{
    if (++it == ender)
...
)
{
    ...
    code_point = static_cast<utfchar32_t>(utf8::internal::mask8(*it));
    UTF8_CPP_INCREASE_AND_RETURN_ON_ERROR(it, ender)
...
)
{
    ...
    UTF8_CPP_INCREASE_AND_RETURN_ON_ERROR(it, ender)
    code_point =
        ((code_point << 18) & 0x1fffff) +
        ((utf8::internal::mask8(*it) << 12) & 0x3ffff)
        ;

    UTF8_CPP_INCREASE_AND_RETURN_ON_ERROR(it, ender)
    code_point = static_cast<utfchar32_t>
    (
...
)
{
    ...
    switch (length)
    {
        ...
        case 1:
            err = utf8::internal::get_sequence_1(it, ender, cp);
            break;

        case 2:
            err = utf8::internal::get_sequence_2(it, ender, cp);
            break;

        case 3:
            err = utf8::internal::get_sequence_3(it, ender, cp);
            break;

        case 4:
            err = utf8::internal::get_sequence_4(it, ender, cp);
            break;
    }
...
template <typename octet_iterator>
inline utf_error
validate_next (octet_iterator & it, octet_iterator ender)
{
    utfchar32_t ignored;
    return utf8::internal::validate_next(it, ender, ignored);
}
...
)
{
    ...
    else
    {
        ...
        else if (is_lead_surrogate(first_word))
        {
            ...
            {
                code_point =
                    static_cast<utfchar32_t>(first_word << 10) +
...
octet_iterator
append (utfchar32_t cp, octet_iterator result)
{
...
inline char *
append (utfchar32_t cp, char * result)
{
...
/**
 *  The caller uses some other kind of output operator - not covered above Note
 *  that in this case we are not able to determine octet_type so we assume it's
 *  utfchar8_t; that can cause a conversion warning if we are wrong.
 */
...
word_iterator
append16 (utfchar32_t cp, word_iterator result)
{
    UTF_CPP_STATIC_ASSERT(sizeof(word_type) >= sizeof(utfchar16_t));
    if (is_in_bmp(cp))
    {
    ...
    {
        /*
         * Code points from the supplementary planes are encoded via surrogate
         * pairs
         */

        *(result++) = static_cast<word_type>(LEAD_OFFSET + (cp >> 10));
        *(result++) =
            static_cast<word_type>(TRAIL_SURROGATE_MIN + (cp & 0x3FF));
    }
    return result;
}
 ...
 * so we assume it's utfchar16_t; that can cause a conversion warning if we are
 * wrong.
 */
...
template <typename octet_iterator>
octet_iterator
find_invalid (octet_iterator start, octet_iterator ender)
{
    ...
    while (result != ender)
    {
        ...
        {
            utf8::internal::validate_next(result, ender)
        };
...
inline const char *
find_invalid (const char * str)
{
    const char * ender { str + std::strlen(str) };
    return find_invalid(str, ender);
}

inline std::size_t
find_invalid (const std::string & s)
{
    std::string::const_iterator invalid { find_invalid(s.begin(), s.end()) };
    return (invalid == s.end()) ?
        std::string::npos : static_cast<std::size_t>(invalid - s.begin());
}

template <typename octet_iterator>
inline bool
is_valid (octet_iterator start, octet_iterator ender)
{
    return (utf8::find_invalid(start, ender) == ender);
}

inline bool
is_valid (const char * str)
{
...
template <typename octet_iterator>
inline bool
starts_with_bom (octet_iterator it, octet_iterator ender)
{
    return
        ((it != ender) && (utf8::internal::mask8(*it++)) == bom[0]) &&
        ((it != ender) && (utf8::internal::mask8(*it++)) == bom[1]) &&
        ((it != ender) && (utf8::internal::mask8(*it))   == bom[2])
        ;
}

inline bool
starts_with_bom (const std::string & s)
{
